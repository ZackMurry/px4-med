...
class Environment:
    ...
    def __init__(self, fixed_layout):
        ...
        self.patient_decay_params = [self._sample_decay_params(w) for w in self.patient_weights]
        self.patient_steps_elapsed = [0] * MAX_PATIENTS  # track time since spawn
        
        
        # Different surival models. Exponential decay (different lamba (decay speeds) for each patient node)
        #                           allows to have variance in how fast they are increasing triage level
        # or can do logistical decay. Each node has their own survival node
        self.updated_patient_weights = self.patient_weights.copy()
    ...
    def _sample_decay_params(self, initial_weight):
        """
        Returns (a, b, thresh_serious, thresh_critical) for each patient.
        a, b         — control the shape of the logistic survival curve
        thresh_serious  — survival below this → weight 2
        thresh_critical — survival below this → weight 3
        """
        if initial_weight == 1:
            a = random.uniform(0.02, 0.05)
            b = random.uniform(3.0,  5.0)
            thresh_serious  = random.uniform(0.50, 0.70)
            thresh_critical = random.uniform(0.15, 0.30)
    
        elif initial_weight == 2:
            a = random.uniform(0.05, 0.10)
            b = random.uniform(2.0,  3.5)
            thresh_serious  = random.uniform(0.45, 0.65)
            thresh_critical = random.uniform(0.15, 0.30)
    
        else:
            a = random.uniform(0.10, 0.20)
            b = random.uniform(1.0,  2.5)
            thresh_serious  = random.uniform(0.40, 0.60)
            thresh_critical = random.uniform(0.10, 0.25)
    
        # Safety guarantee: critical threshold must always be below serious
        thresh_critical = min(thresh_critical, thresh_serious - 0.05)
    
        return (a, b, thresh_serious, thresh_critical)
    ...
    #TODO: have each patient have their own decay function
    # Don't want linear progression 
    # geometric? Find most common in the case of triage
        ...
        if self.new_patient_timer <= 0:
            ...
            for p in range(MAX_PATIENTS):
                if not self.patient_active[p] and not self.patients_delivered[p]:
                    ...
                    self.patient_steps_elapsed[p] = 0                          # add this
                    self.patient_decay_params[p] = self._sample_decay_params(  # add this
                        self.patient_weights[p]
                    )
                    break

        for p in range(MAX_PATIENTS):
            if not self.patient_active[p] or self.patients_delivered[p]:
                continue
            ...
            # Increment time alive for this patient
            self.patient_steps_elapsed[p] += 1
            t = self.patient_steps_elapsed[p]
            a, b, thresh_serious, thresh_critical = self.patient_decay_params[p]

            survival = 1.0 / (1.0 + math.exp(a * t - b))
            
            if survival < thresh_critical:
                self.patient_weights[p] = 3
            elif survival < thresh_serious:
                self.patient_weights[p] = 2
            else:
                self.patient_weights[p] = 1
    ...
    def reset(self):
        ...
        self.patient_steps_elapsed = [0] * MAX_PATIENTS
        self.patient_decay_params = [
            self._sample_decay_params(w) for w in self.patient_weights
        ]
    ...
    def render(self, screen):
        ...
        for p in range(MAX_PATIENTS):
            ...
            # Render weight and timer for active patients
...
def train():
    ...
    RESUME_FROM = None  # set to e.g. "checkpoint_ep1000.pth" to resume

    if RESUME_FROM:
        checkpoint = torch.load(RESUME_FROM)
        ctde_agent.policy_net.load_state_dict(checkpoint['policy_net'])
        ctde_agent.target_net.load_state_dict(checkpoint['target_net'])
        ctde_agent.optimizer.load_state_dict(checkpoint['optimizer'])
    ...
    for episode in range(start_episode, NUM_EPISODES):
        ...
        if episode % 100 == 0:            
            print(
                ...
                f"epsilon: {epsilon:.3f} | Landed: {env.landed} | "
        ...
        if episode % 500 == 0 and episode > 0:
            data.save_to_json(f"checkpoint_ep{episode}.json")
            torch.save({
                'episode': episode,
                'policy_net': ctde_agent.policy_net.state_dict(),
                'target_net': ctde_agent.target_net.state_dict(),
                'optimizer': ctde_agent.optimizer.state_dict(),
                'epsilon': epsilon,
